...
description: 将 C/C++ 项目从 x86 迁移到 ARM（aarch64/鲲鹏），目标是使项目**同时**在 x86 和 ARM 上编译运行（双架构兼容）。支持 Bazel/CMake/Make/Blade/SCons 等主流构建系统。五阶段流程：① 环境检测、修复与准备；② 依赖分析（含 ARM 兼容性探测）；③ 等待用户确认；④ DevKit 扫描 + 构建配置 + 源码适配；⑤ 循环编译验证直到成功。
---
...
> **阶段 A（环境检测、修复与准备）** → **阶段 B（依赖分析）** → **阶段 C（信息收集，等待用户回复）** → **阶段 D（DevKit 扫描 + 源码 & 构建适配）** → **阶段 E（编译验证循环）**
...
> 详细执行步骤见 [environment-prepare.md](environment-prepare.md)，构建工具下载链接见 [build-tools-reference.md](build-tools-reference.md)

**本阶段目标：检测 ARM 环境的编译条件，对不满足要求的项自动修复（安装/升级），输出环境检测与修复报告。**

执行步骤（详见 environment-prepare.md）：

1. **A.1 收集系统环境信息**：获取 x86 和 ARM 环境的 gcc/glibc/内核/OS 版本
2. **A.2 识别构建系统**：扫描项目根目录，确认 Make/CMake/Bazel/Blade/SCons
3. **A.3 检查构建工具版本并修复**：确认 ARM 上的构建工具已安装且版本匹配，未安装或版本不匹配时自动安装（优先内部定制版，回退官方链接）
4. **A.4 检测并修复 Blade 问题**：检查 blade zip 包版本，不支持 arm64 时自动升级（优先内部定制版，回退官方链接）
5. **A.5 检测并修复 Bazel 问题**：对比项目所需版本与系统版本，不一致时自动安装匹配版本（优先内部定制版，回退官方链接）
6. **A.6 检测并修复 Protobuf 问题**：确认 protoc 版本与项目使用的 protobuf 版本一致，不一致时自动源码编译安装
7. **A.7 生成环境检测与修复报告**：输出汇总报告（含环境基础信息表和编译依赖状态表），保存到 `$WORK_DIR/reports/environment-check-report.md`。报告中标注每项的处理状态（已就绪/已处理/待处理），对已处理项记录安装方式和安装路径
...
> ⛔ **硬性约束**：本阶段**必须调用类似 `AskUserQuestion` 工具**向用户提问，**严禁跳过直接进入阶段 D**。即使分析阶段没有发现任何疑问项，也必须向用户展示分析摘要并通过 `AskUserQuestion` 确认"是否继续"，**不得在用户明确回复前自行推进**。

### C.1 整理待确认清单

**阶段 B 完成输出依赖分析报告后**，收集以下所有「需用户决策」的项：
...
**必须使用 `AskUserQuestion` 工具构造问题**，按以下规则组织：

- **每个独立决策点 = 一个 question 条目**
- 选项尽量覆盖所有可行方案（含"跳过/其他方式"兜底选项）
- 即使**全部可自动推断**，也要至少追加一条"确认继续"问题：
...
| 场景 | question 示例 |
|------|---------------|
| 私有库有 ARM 分支 | `"@software ARM 分支 arm64 是否可用？"` 选项：可用 / 不确定 / 跳过 |
| 无 ARM 预编译包 | `"libxxx 无 ARM 版，如何处理？"` 选项：从源码编译 / 提供已有包路径 / 禁用 |
| 版本冲突 | `"bazel 项目需要 x.x，ARM 已装 x.x.x版本，是否兼容？"` 选项：兼容/不兼容 |
| x86 专属功能 | `"AVX 加速模块在 ARM 是否保留？"` 选项：保留(NEON替代) / 禁用 |

### C.3 等待用户回复（严格阻塞）

- **`AskUserQuestion` 调用后，立即停止所有后续操作**，等待用户在 IDE 界面选择或输入
- 收到回复后，将用户选择逐条记录到 `$WORK_DIR/reports/user_decisions.txt`
- 对"跳过"项记录降级方案；对"中止"立即终止流程并输出原因

### C.4 写入确认清单并执行切换

**收到用户回复后**，按 [dependency-analysis.md](dependency-analysis.md) 「用户确认后写入免检清单并执行真实切换」章节：
- 将已确认兼容的依赖写入 `arm_confirmed.md`
- 执行 git fetch + checkout 切换到确认的 ARM 分支并验证
...
- **阶段 A：环境检测、修复与准备**：[environment-prepare.md](environment-prepare.md)
