...
description: 将 C/C++ 项目从 x86 迁移到 ARM（aarch64/鲲鹏），目标是使项目**同时**在 x86 和 ARM 上编译运行（双架构兼容）。支持 Bazel/CMake/Make/Blade/SCons 等主流构建系统。五阶段流程：① 环境检测与准备（subagent）；② 依赖分析（subagent，含 ARM 兼容性探测）；③ 汇总 subagent 待确认项并等待用户确认；④ DevKit 扫描 + 构建配置 + 源码适配；⑤ 循环编译验证直到成功。
---
...
> **阶段 A（环境检测与准备，subagent）** → **阶段 B（依赖分析，subagent）** → **阶段 C（汇总 subagent 待确认项，等待用户回复）** → **阶段 D（DevKit 扫描 + 源码 & 构建适配）** → **阶段 E（编译验证循环）**
