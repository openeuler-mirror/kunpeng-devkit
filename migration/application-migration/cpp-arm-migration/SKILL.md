...
> **阶段 A（环境检测与准备，subagent）** → **阶段 B（依赖分析，subagent）** → **阶段 C（汇总 subagent 待确认项，等待用户回复）** → **阶段 D（DevKit 扫描 → 源码 & 构建适配，严格串行）** → **阶段 E（编译验证循环）** → **阶段 F（迁移总结报告）**
...
主编排在每个阶段的开始和结束时，向时间线日志追加记录，供阶段 F 生成时长统计：

```bash
# 阶段开始时
echo "PHASE_A_START|$(date '+%Y-%m-%d %H:%M:%S')" >> $WORK_DIR/reports/timeline.log

# 阶段结束时
echo "PHASE_A_END|$(date '+%Y-%m-%d %H:%M:%S')" >> $WORK_DIR/reports/timeline.log
```

阶段编号：A / B / C / D / E / F。若某阶段因人工介入未正常结束，记录 `PHASE_X_ABORT|<时间戳>`。
...
>
> `which devkit` 返回空时不允许跳过扫描、也不得自行下载安装，必须通过 `AskUserQuestion` 让用户决策：手动提供安装路径 / 自行下载安装后继续 / 中止（详见 sourcecode-devkit-scan.md D.1.1）。
>
...
> ⚡ **立即 `read_file("migration-summary-report.md")` 获取完整执行步骤。**

**本阶段目标**：汇总整个迁移过程，生成结构化总结报告，包含各阶段执行时长、修改文件清单及修改点描述、重要提示与建议。

**触发条件**：
- 阶段 E 编译成功（E.8 已执行）→ 正常生成完整总结报告
- 阶段 E 触发人工介入（E.7 已输出人工介入报告）→ 生成总结报告，标注"人工介入"状态
...
| [migration-summary-report.md](migration-summary-report.md) | 阶段 F：迁移总结报告生成步骤 |
